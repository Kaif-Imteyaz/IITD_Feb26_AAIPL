...
def train_sft(config: TrainConfig):
    ...
    # Use device_map with single GPU to avoid offloading/meta tensors
    # MI300X has 192GB HBM3 - plenty for 14B model in bf16
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=model_path,
        max_seq_length=config.max_seq_length,
        dtype=torch.bfloat16,
        load_in_4bit=False,  # No 4-bit for ROCm
        device_map={"": 0},  # Force all to GPU 0, avoid meta tensors
        trust_remote_code=True,
    )
    # Ensure model is fully materialized (no meta tensors)
